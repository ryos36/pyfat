...
class FAT(object):
	...
	def __init__(self, fd):
		...
		self.info2 = self.__parse_bootsector32()
		...
		self.__root_dir = ((self.info["num_fats"] * self.info2["fat_sz32"]) *
		...
		if self.info["root_entries"] == 0 :
			self.__root_dir = self.cluster_to_offset(self.info2["root_clus"])
	...
	def next_cluster(self, cluster):
		return self.__next_cluster(cluster)
	...
	def __parse_fat_datetime(self, v1, v2, v3):
		if v1 == 65535 or v2 == 65535 or v3 == 65535 :
			return None
	...
	def __read_dir_entry(self, long_name = None):
		dir_entry = self.fd.read(FAT.DIRSIZE)
		de = unpack("<11sBxBHHHHHHHL", dir_entry)
		# TODO: Handle long filenames
		if de[1] & FAT.Attribute.LONGNAME:
			lfn = unpack("<B10sxxx12s2x4s", dir_entry)
			csum = unpack("<B10sxxB12s2x4s", dir_entry)
			print "checksum:", csum[2]
			n = lfn[0] & 0x3f
			long_name = lfn[1] + lfn[2] + lfn[3];
			while n != 1:
				lfn = unpack("<B10sxxx12s2x4s", self.fd.read(FAT.DIRSIZE))
				long_name = lfn[1] + lfn[2] + lfn[3] + long_name;
				n = lfn[0] & 0x3f
			i = 0
			c = long_name[i]
			nln = ""
			while ord(c) != 0:
				...
				i += 2
				c = long_name[i]
			return self.__read_dir_entry(nln)
		else:
			return {
				...
				"modified": self.__parse_fat_datetime(0, de[7], de[8]),
				"cluster": de[9] + de[6] * 65536,
				"size": de[10],
